...
import glob
import os
from definitions import ROOT_DIR, SEED
...
class ImageDataset:
    ...
    def _load(self, img_file, img_type):
    ...
    def _load_paired_image(self, input_img_file, output_img_file):
        input_img = self._load(input_img_file, img_type="jpg")
        norm_input_img = self._normalize(input_img)

        output_img = self._load(output_img_file, img_type="png")
    ...
    def _create_dataset(self, shuffle=True):
        ...
        if shuffle:
            dataset = dataset.shuffle(len(self.sample_images_urls), reshuffle_each_iteration=False, seed=SEED)
